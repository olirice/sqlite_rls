...
#[path = "compat.rs"]
mod compat;
use compat::empty_params;

#[tokio::main]
async fn main() -> Result<()> {
    // Open an in-memory database using the Builder API
    let db = libsql::Builder::new_local("file::memory:")
        .build()
        .await?;
...
async fn query_with_rls(conn: &Connection, sql: &str, user_id: i64) -> Result<Rows> {
    ...
    // Parse SQL to AST - using the updated API
    ...
    if let Statement::Query(query) = stmt {
        if let ast::SetExpr::Select(select) = &mut *query.body {
            ...
            let condition = Expr::BinaryOp {
                left: Box::new(Expr::Identifier(ast::Ident {
