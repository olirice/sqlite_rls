...
async fn main() -> Result<()> {
    ...
    let db = Database::open("file::memory:")?;
...
async fn query_with_rls(conn: &Connection, sql: &str, user_id: i64) -> Result<Rows> {
    ...
    let ast = Parser::parse_sql(sql, &dialect)?;
    ...
    let rows = conn.query(&transformed_sql, []).await?;
    Ok(rows)
}
    ...
    if let Statement::Query(query) = stmt {
        if let ast::SetExpr::Select(select) = &mut *query.body {
            ...
            let condition = Expr::BinaryOp {
                ...
                right: Box::new(Expr::Value(ast::Value::Number(user_id.to_string()))),
            };
